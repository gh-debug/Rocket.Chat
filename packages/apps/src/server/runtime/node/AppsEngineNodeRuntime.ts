...
const baseDebug = debugFactory('appsEngine:runtime:node');
...
export class NodeRuntimeSubprocessController extends EventEmitter implements IRuntimeController {
	private node: child_process.ChildProcess | undefined;
	...
	public spawnProcess(): void {
		try {
			...
			const options = [
				'--permission',
				...allowedDirs.map((path) => `--allow-fs-read=${path}`),
			...
			this.node = child_process.spawn(this.nodeBin, options, environment);
			this.messenger.setReceiver(this.node);
			this.livenessManager.attach(this.node);

			this.debug('Started subprocess %d with options %s and env %s', this.node.pid, inspect(options), inspect(environment));
	...
	public async killProcess(): Promise<boolean> {
		...
		if (this.node.kill('SIGKILL')) {
			// Let's wait until we get confirmation the process exited
			await new Promise<void>((r) => this.node.on('exit', r));
			killed = true;
		} else {
	...
	public async restartApp() {
		...
		try {
			...
			await this.setupApp();
			logger.info({ msg: 'New subprocess successfully spawned', pid: this.node.pid });
	...
	private setupListeners(): void {
		...
		this.node.stderr.on('data', this.parseError.bind(this));
		this.node.on('error', (err) => {
			this.state = 'invalid';
			console.error(`Failed to startup Deno subprocess for app ${this.getAppId()}`, err);
		});

		this.node.once('exit', (code) => this.emit('processExit', code));

		this.once('ready', this.onReady.bind(this));

		void this.parseStdout(this.node.stdout);
	}
